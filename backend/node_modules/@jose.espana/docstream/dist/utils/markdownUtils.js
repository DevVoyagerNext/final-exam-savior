 ...
 */
function astToMarkdown(content, config) {
 ...
 */
function renderHeading(node, nl) {
 ...
 */
function renderParagraph(node, nl) {
 ...
 */
function renderInlineChildren(node, nl) {
    if (!node.children)
        return node.text || '';
 ...
 */
function extractCellText(cell, nl) {
    if (cell.children && cell.children.length > 0) {
        return cell.children.map(child => {
            if (child.type === 'text')
                return renderText(child);
            if (child.children)
                return extractCellText(child, nl);
            return child.text || '';
        }).filter(t => t !== '').join(' ');
    }
    return cell.text || '';
...
function renderSheet(node, nl) {
    ...
    if (node.children) {
        ...
        if (rows.length > 0) {
            const tableNode = { type: 'table', children: rows };
 ...
 */
function renderHeaderFooter(node, nl, label) {
    const parts = [];
    if (node.children) {
        for (const child of node.children) {
            const rendered = renderNode(child, nl);
 ...
 */
function renderChildren(node, nl) {
    if (node.children) {
