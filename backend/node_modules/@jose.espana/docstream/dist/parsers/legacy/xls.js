 ...
 */
function readShortBiffString(buf, offset) {
 ...
 */
function parseSST(sstData, continueBuffers) {
    const strings = [];
    if (sstData.length < 8)
        return strings;
    ...
    for (let i = 0; i < uniqueCount && pos < combined.length; i++) {
        ...
        let phoneticSize = 0;
        if (hasRichText) {
            if (strPos + 2 > combined.length)
                break;
            richTextRunCount = combined.readUInt16LE(strPos);
            strPos += 2;
        }
        if (hasPhonetic) {
            if (strPos + 4 > combined.length)
                break;
 ...
 */
async function parseXls(fileBuffer, config) {
    ...
    for (let sheetIdx = 0; sheetIdx < boundSheets.length; sheetIdx++) {
        ...
        pos = sheet.bofOffset;
        if (pos + 4 > workbookStream.length)
            continue;
        // Verify BOF
                ...
                if (cell.xfIndex < xfRecords.length) {
                    ...
                    if (xf.fontIndex < fonts.length) {
                        ...
                        if (font) {
                            ...
                            if (font.size)
                                formatting.size = `${font.size / 20}pt`;
                            if (font.name)
