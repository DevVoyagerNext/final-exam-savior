...
function parseFIB(mainStream) {
    ...
    function readFcLcb(index) {
        if (index >= cbRgFcLcb)
            return [0, 0];
        const off = fcLcbOffset + index * 8;
        if (off + 8 > mainStream.length)
            return [0, 0];
 ...
 */
function parseTextPieces(tableStream, fib, mainStream) {
    if (fib.lcbClx === 0) {
 ...
 */
function parseStylesheet(tableStream, fib) {
    const styles = [];
 ...
 */
function parseParagraphProperties(tableStream, mainStream, fib, textLength) {
    const paragraphs = [];
    if (fib.lcbPlcfbtePapx === 0)
        return paragraphs;
    if (fib.fcPlcfbtePapx + fib.lcbPlcfbtePapx > tableStream.length)
        return paragraphs;
    ...
    for (let i = 0; i < numEntries; i++) {
        ...
        if (pageOffset + 512 > mainStream.length)
            continue;
        ...
        for (let j = 0; j < crun; j++) {
            ...
            if (papxPos + 2 > mainStream.length)
                continue;
 ...
 */
function parseFontTable(tableStream, fib) {
    const fonts = [];
 ...
 */
function parseCharacterProperties(fib, mainStream, tableStream, fontTable) {
    const runs = [];
    if (fib.lcbPlcfbteChpx === 0)
        return runs;
    if (fib.fcPlcfbteChpx + fib.lcbPlcfbteChpx > tableStream.length)
        return runs;
...
function mapChpxToCp(charRuns, textPieces) {
    const cpRanges = [];
 ...
 */
function mapPapxToCp(papxInfo, textPieces) {
    const cpParagraphs = [];
 ...
 */
function splitIntoFormattedRuns(text, paragraphCpStart, cpRanges) {
    ...
    for (const range of cpRanges) {
        if (range.cpStart >= paragraphCpEnd)
            break;
        if (range.cpEnd <= paragraphCpStart)
            continue;
 ...
 */
async function parseDoc(fileBuffer, config) {
    // 1. Open OLE2 container
    const ole2 = (0, ole2_1.parseOLE2)(fileBuffer);
    ...
    function isListStyle(istd) {
        ...
        if (!style || !style.name)
            return undefined;
        const name = style.name.toLowerCase().replace(/\0/g, '');
        if (name.includes('list bullet') || name.includes('listbullet'))
            return 'unordered';
        if (name.includes('list number') || name.includes('listnumber'))
            return 'ordered';
        if (name.includes('list paragraph') || name.includes('listparagraph'))
            return undefined; // Could be either
        return undefined;
    }
    ...
    for (let paraIdx = 0; paraIdx < paragraphTexts.length; paraIdx++) {
        ...
        for (let pi = 0; pi < papxInfo.length; pi++) {
            ...
            if (papxInfo[pi].cpStart > paraCpStart)
    ...
    return {
        ...
        toMarkdown: () => (0, markdownUtils_1.astToMarkdown)(content, config),
    };
}
...
// ============================================================================
function buildTableNode(rows, config) {
    const delimiter = config.newlineDelimiter ?? '\n';
    const rowNodes = [];
    for (let r = 0; r < rows.length; r++) {
        ...
        for (let c = 0; c < cells.length; c++) {
            if (cells[c].metadata) {
                cells[c].metadata.row = r;
                cells[c].metadata.col = c;
            }
        }
