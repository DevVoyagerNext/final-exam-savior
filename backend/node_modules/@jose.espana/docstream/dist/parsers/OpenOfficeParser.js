...
const parseOpenOffice = async (buffer, config) => {
    ...
    const parseStyles = (xmlString) => {
        ...
        for (const style of styles) {
            ...
            const drawPageProps = (0, xmlUtils_1.getElementsByTagName)(style, "style:drawing-page-properties")[0];
            if (drawPageProps) {
                const fill = drawPageProps.getAttribute("draw:fill");
                if (fill && fill !== 'none') {
                    const bg = { fillType: 'none' };
                    if (fill === 'solid') {
                        bg.fillType = 'solid';
        ...
        // ODP: Presentation
            ...
            if (presentation) {
                ...
                for (let i = 0; i < pages.length; i++) {
                    const page = pages[i];
                    const slideMeta = { slideNumber: i + 1 };
                    // Apply slide background from drawing-page style
                    const drawStyleName = page.getAttribute("draw:style-name");
