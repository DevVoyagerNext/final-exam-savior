...
const parseWord = async (buffer, config) => {
            ...
            if (node.nodeName === 'w:r') {
                ...
                if (config.extractAttachments) {
                    ...
                    for (const imgNode of allImages) {
                        ...
                        const graphicData = (0, xmlUtils_1.getElementsByTagName)(imgNode, "a:graphicData")[0];
                        if (graphicData) {
                                ...
                                if (cChart) {
                                    ...
                                    if (chartRId && relsMap[chartRId]) {
                                        ...
                                        const chartNode = {
                                            type: 'chart',
                                            text: '',
                                            metadata: { attachmentName: chartName }
                                        };
                                        if (config.includeRawContent) {
    ...
    const parseTable = (tblNode) => {
        ...
        for (let rIndex = 0; rIndex < trNodes.length; rIndex++) {
            ...
            for (let cIndex = 0; cIndex < tcNodes.length; cIndex++) {
                ...
                const tcPr = (0, xmlUtils_1.getElementsByTagName)(tcNode, "w:tcPr")[0];
                // gridSpan for horizontal merge
                let colSpan = 1;
                if (tcPr) {
                    const gridSpanNode = (0, xmlUtils_1.getElementsByTagName)(tcPr, "w:gridSpan")[0];
                    if (gridSpanNode) {
                ...
                if (tcPr) {
                    const vMergeNode = (0, xmlUtils_1.getElementsByTagName)(tcPr, "w:vMerge")[0];
                    if (vMergeNode) {
                ...
                const cellMeta = { row: rIndex, col: cIndex };
                if (colSpan > 1)
                    cellMeta.colSpan = colSpan;
                const cellNode = {
                    type: 'cell',
                    text: cellText,
        ...
        for (const col in vMergeState) {
            ...
            if (state.count > 1) {
                state.node.metadata.rowSpan = state.count;
            }
        }
        return {
            type: 'table',
    ...
    for (const file of files) {
        ...
        if (file.path.match(chartFileRegex))
            continue;
        if (file.path.match(chartRelsRegex))
            continue;
        ...
        const bgElement = (0, xmlUtils_1.getElementsByTagName)(doc, "w:background")[0];
        if (bgElement) {
            ...
            const vFill = (0, xmlUtils_1.getElementsByTagName)(bgElement, "v:fill")[0];
            if (vFill) {
    ...
    const footerRefs = {};
    if (documentFile) {
        const docXml = (0, xmlUtils_1.parseXmlString)(documentFile.content.toString());
        const sectPr = (0, xmlUtils_1.getElementsByTagName)(docXml, "w:sectPr");
        for (const sect of sectPr) {
            const hRefs = (0, xmlUtils_1.getElementsByTagName)(sect, "w:headerReference");
            ...
            const fRefs = (0, xmlUtils_1.getElementsByTagName)(sect, "w:footerReference");
    ...
    for (const file of files) {
        if (file.path.match(headerFileRegex)) {
            const xml = (0, xmlUtils_1.parseXmlString)(file.content.toString());
            const children = [];
