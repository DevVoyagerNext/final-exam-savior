...
const parsePowerPoint = async (buffer, config) => {
    ...
    const appMetadata = appPropsFile ? (0, xmlUtils_1.parseAppMetadata)(appPropsFile.content.toString()) : {};
    // Sort files
    files.sort((a, b) => {
    ...
    for (const file of files) {
        ...
        if (!isNote) {
            const bg = (0, xmlUtils_1.getElementsByTagName)(xml, "p:bg")[0];
            if (bg) {
                const bgPr = (0, xmlUtils_1.getElementsByTagName)(bg, "p:bgPr")[0];
                if (bgPr) {
                    const background = { fillType: 'none' };
                    // Solid fill
                    const solidFill = (0, xmlUtils_1.getElementsByTagName)(bgPr, "a:solidFill")[0];
                    if (solidFill) {
                    ...
                    // Image fill (blipFill)
                    const blipFill = (0, xmlUtils_1.getElementsByTagName)(bgPr, "a:blipFill")[0];
                    if (blipFill) {
                        background.fillType = 'image';
                        const blip = (0, xmlUtils_1.getElementsByTagName)(blipFill, "a:blip")[0];
                        if (blip) {
                            const rId = blip.getAttribute("r:embed");
                    ...
                    // Gradient fill
                    const gradFill = (0, xmlUtils_1.getElementsByTagName)(bgPr, "a:gradFill")[0];
                    if (gradFill) {
                        background.fillType = 'gradient';
                        const gsLst = (0, xmlUtils_1.getElementsByTagName)(gradFill, "a:gsLst")[0];
                        if (gsLst) {
                            const stops = (0, xmlUtils_1.getElementsByTagName)(gsLst, "a:gs");
                            const colors = [];
                            for (const gs of stops) {
                                const srgbClr = (0, xmlUtils_1.getElementsByTagName)(gs, "a:srgbClr")[0];
                                if (srgbClr) {
                                    const val = srgbClr.getAttribute("val");
                                    if (val)
                                        colors.push('#' + val);
                            ...
                            if (colors.length > 0)
                                background.gradientColors = colors;
                        }
                    }
                    // Pattern fill
                    const pattFill = (0, xmlUtils_1.getElementsByTagName)(bgPr, "a:pattFill")[0];
                    if (pattFill) {
                        background.fillType = 'pattern';
                        const fgClr = (0, xmlUtils_1.getElementsByTagName)(pattFill, "a:fgClr")[0];
                        if (fgClr) {
                            const srgbClr = (0, xmlUtils_1.getElementsByTagName)(fgClr, "a:srgbClr")[0];
                            if (srgbClr) {
                                const val = srgbClr.getAttribute("val");
                                if (val)
                                    background.color = '#' + val;
                    ...
                    // No fill
                    const noFill = (0, xmlUtils_1.getElementsByTagName)(bgPr, "a:noFill")[0];
                    if (noFill) {
                        background.fillType = 'none';
                    }
                    if (background.fillType !== 'none') {
                        slideNode.metadata.background = background;
                    }
                }
                // Background reference (p:bgRef) - theme-based solid color
                const bgRef = (0, xmlUtils_1.getElementsByTagName)(bg, "p:bgRef")[0];
                if (bgRef && !bgPr) {
                    const srgbClr = (0, xmlUtils_1.getElementsByTagName)(bgRef, "a:srgbClr")[0];
                    if (srgbClr) {
                        const val = srgbClr.getAttribute("val");
                        if (val) {
                            slideNode.metadata.background = {
                                fillType: 'solid',
                                color: '#' + val
                            };
                        }
